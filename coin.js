...
(function () {
	...
	coinjs.host='https://api.blockcypher.com/v1/btc/test3/';//'https://testnet.blockchain.info/';
	...
	*/
	...
	coinjs.addressBalance = function(address, callback){
		coinjs.ajax(coinjs.host+'addrs/'+address+'?unspentOnly=true&confirmations=1&r='+Math.random(),callback);
	}
	...
	coinjs.transaction = function() {
		...
		r.addinput = function(txid, index, script, sequence){
			...
			console.log('addinput '+JSON.stringify(o));
		...
		r.addoutput = function(address, value){
			...
			console.log('addoutput '+JSON.stringify(o));
		...
		r.adddataofstring=function(str){
			var data = "";
		...
		r.listUnspent = function(address, callback) {
			coinjs.ajax(coinjs.host+'addrs/'+address+'?unspentOnly=true&includeScript=true&confirmations=1&r='+Math.random(),callback);
		}

		/* add unspent to transaction */
		r.addUnspent = function(address, totalpay,callback, script, segwit, sequence){
			var self = this;
			this.listUnspent(address, function(data){
				...
				try{
					var unspent = data.txrefs;
					for(var i=0;i<unspent.length;i++){
						if( unspent[i].value*1 < 1 ){
							continue;
						}
						var txhash = unspent[i].tx_hash;//tx_hash;
						var n = unspent[i].tx_output_n;
						var scr = unspent[i].script;
					...
					console.log('listUnspent Error!'+e);
					return callback({});
				}
			});
		}
		...
		r.broadcast = function(callback, txhex){
			...
			$.post(coinjs.host+'txs/push', JSON.stringify(pushtx)).success(function(d) {callback(d)}).error(e=>{
				callback(e);
			});
		}
	...
	coinjs.ajax = function(url, callback){
		// $.get(coinjs.api+encodeURIComponent(url),function(data){
		// 	callback(data);
		// });
		
		$.get(url+'&cors=true',function(data){
			callback(data);
		});
	}
